...
Здравствуйте. Я [Євген Шемет](https://www.linkedin.com/in/yevhene/) - розробник, доповідач та з цього року викладач ВНТУ. Я доповідав на [Vinnytsia.js](http://vinnytsiajs.org/) про [React 16](https://present-fiber.herokuapp.com/). І мене нещодавно запросили написати статтю на Dou. Статтю я пишу вперше, тому впевнений будуть зауваження та побажання, відправляйте їх, якщо ваша ласка, в особисті повідомлення.
Поки я писав статтю, React 16 вже [вийшов](https://facebook.github.io/react/blog/2017/09/26/react-v16.0.html). Але від цього моя стаття не стала менш актуальною.
...
## Наразі
React, для забезпечення вискої швидкості роботи виористовує технологію *Vitrual DOM*: В пам'яті підтримується спрощена копія DOM (Document Object Model), де за вузлами закріплені конкретні екземпляри (instance) компонентів, що їми керують. Коли змінюється стан екземпляру, відбувається процес оновлення, що складається з етапів:
...
Зараз оновлення використовується алгоритм, що його заднім числом назвали *Stack*. І в нього, з часом, виявився значний недолік: він працює простим пошуком в глибину і його робота неперервна. А оскільки в браузері все виконується в одному потоці, то під час оновлення, все інше має зачекати. В випаду високоприорітетних оновлень, наприклад анімаціі, це може стати відчутним клопотом для користувача.
...
Дерево поступово розгортується і на його основі створюється нове дерево. Там де є оновлення - клонуються елементи і вносяться зміни. Там де оновлень немає - використовуються існуючі елементи.
...
- Якщно новий стан буде залежати від поточного стану, то необхідно використовувати параметр зворотнього виклику `prevState`, замість `this.state`. Тому що його може бути викликано декілька разів.
...
## Приоритизація
Оновлення всередині вже приорітизуються але цей процесс щє далекий від ідеалу, крім того очікується більше контролю над цим процесом. Приорітети:
...
Оскільки візуалізація розбита на дві фази, є можливість чітко знати, коли все необхідне завантажене, обраховане і готове до відображення. Це відкриває шлях, в майбутньому, до потокового завантаження превізуалізованих на сервері компонентів. Також має спростити етап завантаження великих аплікацій.
...
Будьте уважні та обережні, якщо ви використовуєте відкладені оновлення. Деякі методи життєвого циклу, під час одного оновлення, можуть викликатись двічі або більше разів. Пов'язано це з тим, що оновлення може бути відкладено, через більш нагальні оновлення, а потім буде переобраховано. Це методи фази *узгодження* (reconciliation).
- componentWillMount
